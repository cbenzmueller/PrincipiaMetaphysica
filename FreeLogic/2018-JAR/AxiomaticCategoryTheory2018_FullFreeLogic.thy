...
approach; here we simply reuse this modeling. Note that the connectives and quantifiers of free logic are displayed below in bold-face fonts. 
...
axiomatization where fStarAxiom: "\<not>E(\<^bold>\<star>)" --\<open> \<open>\<^bold>\<star>\<close> is a ``non-existing'' object in \<open>D\<close>. \<close>
...
(* Commented Part: This model can no longer be constructed, if * is assumend a non-existing object 
