...
typedecl i    -- "the type for indiviuals" 

consts f_A :: "i\<Rightarrow>\<sigma>" ("\<A>")   (* Existence *)
consts f_star :: "i" ("\<^bold>\<star>")   (* Undefinedness *)
...
abbreviation f_all :: "(i\<Rightarrow>\<sigma>)\<Rightarrow>\<sigma>" ("\<^bold>\<forall>")                 
 where "\<^bold>\<forall>\<Phi> \<equiv> \<forall>x. \<A>(x)\<longrightarrow>\<Phi>(x)"   
abbreviation f_all_bind :: "(i\<Rightarrow>\<sigma>)\<Rightarrow>\<sigma>" (binder "\<^bold>\<forall>" [8] 9) 
 where "\<^bold>\<forall>x. \<phi>(x) \<equiv> \<^bold>\<forall>\<phi>"
abbreviation f_that :: "(i\<Rightarrow>\<sigma>)\<Rightarrow>i" ("\<^bold>I") 
 where "\<^bold>I \<Phi> \<equiv> if \<exists>x. \<A>(x) \<and> \<Phi>(x) \<and> (\<forall>y. (\<A>(y) \<and> \<Phi>(y)) \<longrightarrow> (y = x)) 
               then THE x. \<A>(x) \<and> \<Phi>(x) 
               else \<^bold>\<star>"
abbreviation f_that_b :: "(i\<Rightarrow>\<sigma>)\<Rightarrow>i"  (binder "\<^bold>I" [8] 9) 
...
abbreviation f_equals :: "i\<Rightarrow>i\<Rightarrow>\<sigma>" (infixr "\<^bold>="56)       
 where "x\<^bold>=y \<equiv> x=y"
abbreviation f_exi :: "(i\<Rightarrow>\<sigma>)\<Rightarrow>\<sigma>" ("\<^bold>\<exists>")                 
 where "\<^bold>\<exists>\<Phi> \<equiv> \<^bold>\<not>\<^bold>\<forall>(\<lambda>y.\<^bold>\<not>(\<Phi> y))"
abbreviation f_exi_b :: "(i\<Rightarrow>\<sigma>)\<Rightarrow>\<sigma>"  (binder "\<^bold>\<exists>" [8]9)  
...
consts f_r  :: "i\<Rightarrow>i\<Rightarrow>\<sigma>" (infixr "\<^bold>r" 70)    (* Scott considers just one binary relation r *) 
...
lemma "(x \<^bold>r x \<^bold>\<rightarrow> x \<^bold>r x) \<^bold>\<rightarrow> (\<^bold>\<exists>y. y \<^bold>r y \<^bold>\<rightarrow> y \<^bold>r y)" nitpick oops
 (* not valid *)
lemma "((x \<^bold>r x \<^bold>\<rightarrow> x \<^bold>r x) \<^bold>\<and> (\<^bold>\<exists>y::i. y \<^bold>= y)) \<^bold>\<rightarrow> (\<^bold>\<exists>y. y \<^bold>r y \<^bold>\<rightarrow> y \<^bold>r y)" by simp
