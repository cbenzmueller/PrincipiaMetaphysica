...
In this contribution to AFP we show how free logic can be ``implemented'' in Isabelle/HOL.
Our approach can of course be transferred to other proof assistants for classical 
higher-order logic (HOL) @{cite "B5"}. The proposed solution 
...
In this article we in fact introduce free higher-order logic. That is, instead of just considering 
the above construction for first-order domains  \<open>\<^bold>D\<close> and \<open>\<^bold>E\<close>, we apply these constructions for 
to domains \<open>\<^bold>D\<^sub>t\<close> and \<open>\<^bold>E\<^sub>t\<close> for all types \<open>\<^bold>t\<close>. To achieve this we work with polymorphic definitions 
resp. abbreviations.
\<close>
...
text \<open>
We start out with introducing a type \<open>\<^bold>t\<close>. The domain of objects associated with this 
this type will serve as the domain of raw objects \<open>\<^bold>D\<^sub>t\<close>, cf. Fig~\ref{fig1}. Moreover, we 
introduce an existence predicate \<open>\<^bold>E\<^sub>t\<close> on type i. As mentioned, \<open>\<^bold>E\<^sub>t\<close> 
characterises the subset of existing objects in \<open>\<^bold>D\<^sub>t\<close>. Next, we declare a special (polymorphic) 
constant symbol  star \<open>\<^bold>\<star>\<close>, which is intended to denote a distinguished ``non-existing'' element 
of \<open>\<^bold>D\<^sub>t\<close>. 
\<close>

consts fExistence:: "'a\<Rightarrow>bool" ("\<^bold>E")    \<comment> "Polymorphic existence predicate."
consts fStar:: "'a" ("\<^bold>\<star>")   \<comment> "Polymorphic distinguished symbol for undefinedness"

text \<open>
We postulate that \<open>\<^bold>\<star>\<close> is a ``non-existing'' object in \<open>\<^bold>D\<^sub>t\<close>. 
\<close>
