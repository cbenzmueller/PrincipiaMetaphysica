...
\title{Automating Free Logic in HOL, with an Application in Category Theory}
...
Isabelle/HOL.  We also address the relation of our axioms systems to
...
We now formally introduce the syntax and semantics of Scott's \cite{Scott67} free logic as
...
\item given formulas $r$ and $s$ of $\FFOL$, then $\neg r$, $r
  \Impl s$ and $\forall x\lambdot r$ are (compound) formulas of $\FFOL$
...
\begin{definition}[Dual-domain semantics of $\FFOL$]
 ...
 $0$-ary predicate symbols (propositions) to $\True$ or $\False$, 
  ...
  define the interpretation function $\|\cdot\|^{M,g}$ for terms and formulas of
  $\FFOL$ as follows:
\noindent
\begin{description}
\item{Terms}
...
\item $\| q \|^{M,g} = I(q)$, where $q\in\P$ is an $0$-ary predicate
  symbol
\item $\| t_1 = t_2 \|^{M,g} = \True$ iff $\| t_1 \|^{M,g} = \| t_2
  \|^{M,g}$ (this basic notion of primitive equality on $D$ implies that equations such as $1/0 = 1/0$ are
...
\item $\| p(t_1,\ldots,t_n) \|^{M,g} = \True$ iff $(\| t_1 \|^{M,g},\ldots,\|
  t_n \|^{M,g}) \in I(p)$ for $n$-ary ($n\geq 1$) predicate symbols
  $p\in\P$
\item $\| \neg r \|^{M,g} = \True$ iff $\| r \|^{M,g} = \False$
\item $\| r \Impl s \|^{M,g} = \True$ iff $\| r \|^{M,g} = \False$
  or $\| s \|^{M,g} = \True$
\item $\| \forall x\lambdot r  \|^{M,g} = \True$ iff for all $d \in \ex$ we
...
\begin{definition}[Validity]\label{ffollvalid}
  ...
  $M \models^\FFOL s_o$, iff $M,g \models^\FFOL s_o$ for all assignments
  $g$. Finally, a formula $s_o$ is called \emph{valid}, which we
  denote by $\models^\FFOL s_o$, iff $s_o$ is valid for all $M$. Moreover,
...
choice is defined by the following
grammar:\footnote{It is well known that we could work with a much smaller set
  of logical connectives, see e.g. \cite{Andrews02}. The choice here is motivated by the
  signature supported in higher-order automated theorem provers such
  as LEO-II \cite{LEO-II} and Leo-III \cite{Leo-III}.}
\begin{align*} 
...
\begin{definition}[Evaluation Function]\label{holvalue}
...
\item $\hol{\|s_\alpha = t_\alpha\|^{M,g}} = {\True}$ iff $\hol{\|s_\alpha\|^{M,g}} = \hol{\|t_\alpha\|^{M,g}} $
\item $\hol{\|(\neg_{o\ar o}\, s_o)_o\|^{M,g}} = {\True}$ iff $\hol{\|s_o\|^{M,g}} = {\False}$
\item $\hol{\|(({\vee_{o\ar o\ar o}}\,s_o)\,t_o)_o\|^{M,g}} =
  {\True}$ iff\, $\hol{\|s_o\|^{M,g}} = {\True}$ or $\hol{\|t_o\|^{M,g}}
  = {\True}$
\item \sloppy $\hol{\|(\forall_{(\alpha\ar o)\ar o}(\lam{X_\alpha}
    s_o))_o\|^{M,g}} = {\True}$ iff for all $\hol{d}\in
...
\begin{definition}[Standard model and Henkin model]\label{homlhenkinmodel}
...
under assignment $\hol{g}$ iff $\hol{\|s_o\|^{M,g}} = {\True}$; this is also denoted
...
In this embedding raw domains $D$ of a $\FFOL$ model structures are
identified with the $\HOL$ domains $\hol{D_i}$.  The subdomain $E$ of
existing objects is characterized in the embedding by a $\HOL$
predicate $\hol{\ex}$ of type $\hol{ i\ar o}$. Hence, we assume in the
remainder that a respective uninterpreted constant symbol $\hol{\ex}$
is given in the signature of $\HOL$. Moreover, we assume that an
uninterpreted constant symbol $\hol{\star}$ of type $i$ be in the
signature of $\HOL$. We assume that $\hol{\|E \star\|^{M,g}=F}$ for
all $\hol{M,g}$, i.e. that the element denoted by $\hol{\star}$ is not
an element of the domain of existing objects denoted by $\hol{\ex}$.
...
$\map{f(t^1,\ldots,t^n)}$    & := & $(\map{f}\ \map{t^1} \ldots \map{t^n})$  \quad
 ...
 & & &  where $\map{f}=\hol{f_{\underbrace{{i \ar
               \ldots  \ar i} \ar }_{n\geq o} i}} \in \map{F}$\\
...
$\map{p(t^1,\ldots,t^n)}$    & := & $(\map{p}\ \map{t^1} \ldots \map{t^n})$  \quad
   ...
   & & & where $\map{p}=\hol{p_{\underbrace{{i \ar
               \ldots  \ar i} \ar }_{n\geq o} o}} \in \map{P}$\\
...
$\map{\Descr x\lambdot r}$ & := &  $\hol{\textbf{IfThenElse}}$ \\
  & & \multicolumn{2}{c}{\quad $\hol{(\exists X_i\lambdot \ex X \wedge} \map{r}\ \hol{\wedge (\forall
...
\HOL domains $\hol{D_i}$, i.e. $\map{D} = \hol{D_i}$): Let $g$ be a variable assignment for
  ...
  $\map{g}(X_i) = \map{g} \map{x} = g(x)$ 
for all $\hol{X_i}\in \map{V}$. 
Finally, $\map{g}$ is extended to an assignment for arbitrary
variables $\hol{Y_\alpha} \notin \map{V}$  by choosing
$\map{g}(\hol{Y_\alpha}) = \hol{d}\in \hol{D_\alpha}$ arbitrary (for
all types $\alpha$).
...
chosen as (not necessarily full) sets of functions from
$\hol{D_\alpha}$ to $\hol{D_\beta}$. Remember, however, that the
choice of $\hol{D_{\alpha\ar\beta}}$ must always ensure that 
the valuation function 
 $\hol{\|\cdot\|^{\map{M},\map{g}}}$ remains total, i.e. that
 all terms denote. In particular, it is required that $\hol{D_{i\ar
     o}}$ contains the element $\hol{I \ex}$ as characterized below.
\item Let $X^i_i = \map{x^i}$
  for $x^i\in V$.  The interpretation function $\hol{I}$ of $\map{M}$
  is chosen as follows:
\begin{itemize}
\item  $\hol{I \star_i} = \star \in D$
\item  For all
    $\hol{d} \in \hol{D_i} = \map{D}$ we have: $(\hol{I \ex_{i\ar o}})(\hol{d}) =
    \hol{T}$  if and only if $\hol{d} \in E$. Note that this implies $(\hol{I \ex})(\hol{\star_i}) = \hol{F}$.
\item \sloppy For all $\hol{f_{{\underbrace{{i \ar
               \ldots  \ar i} \ar }_{n\geq o} i}}} = \map{f} \in \map{F}$ we
       have: $(\hol{I} \hol{f})(\hol{d^1},\ldots,\hol{d^n}) = (I f)(\hol{d^1},\ldots,\hol{d^n})$ for
       all $\hol{d^i}\in \hol{D_i} = \map{D}$ ($i=1,\ldots,n$ and
       $n\geq 0$).
\item \sloppy For all $\hol{p_{{\underbrace{{i \ar
               \ldots  \ar i} \ar }_{n\geq o} 0}}} = \map{p} \in \map{P}$ we
...
\item For all other constants $\hol{c_\alpha}$, choose $\hol{I
  ...
  no other constant symbols given, except for the symbols in
  $\map{F}$ and $\map{P}$, the symbols $\hol{\ex_{i\ar o}}$  and $\hol{\star_{i}}$ and the logical connectives.}
\end{itemize}
\end{itemize}

It is not hard to verify that ${H}^{M}$ is a Henkin model.\footnote{In
  $\map{M}$ we have merely fixed $D_i=D$ and the interpretations of
  $\ex$ and $\star$.  These
  ...
  function spaces have been chosen in frame $\{D_\alpha\}_{\alpha \in
    {T}}$. Andrews \cite{Andrews72b} discusses criteria that are sufficient
...
\begin{proof} 
  ...
  the induction hypothesis. The arguments for $s=p\in P$ where $p$ is
  ...
  which $r$ holds) or there is no such $d\in \ex$. In the first case
  we have $\| \Descr x\lambdot r \|^{M,g} = d \in \ex$. By the
  definition of the embedding $\map{\ }$, definition of $\map M$ and
  $\map g$, induction hypothesis and a series of tedious evaluation
  steps in \HOL we get
  ...
  definition of the embedding $\map{\ }$, the definition of $\map M$
  and $\map g$, induction hypothesis and a series of tedious
  evaluation steps in \HOL to verify that
  $\| \map{\Descr x\lambdot r} \|^{\map M, \map g} = \hol{\star} =
  \star$. The remaining cases are similar (actually simpler) and left to the reader.
\end{proof}
\end{lemma}
...
For all formulas $s\in FFOL$ we have
 ...
 (Completeness, $\rightarrow$) The proof is again by
    contraposition and analogous to above.
\end{proof}
\end{theorem} 
...
In the remainder of this article we present an application of our
framework for the exploration of axioms systems in category theory. In
this context description is not required and we hence omit it in our 
theory. Figure \ref{pic1} shows the embedding of $\FFOL$ in $\HOL$,
encoded in Isabelle/HOL, 
that we have employed in our experiments. A related Isabelle/HOL
encoding of the above theory with description has been presented and  discussed in
\cite{ICMS}. 

\begin{figure}[htp]
 \includegraphics[width=\textwidth]{EmbeddingPic}
 \caption{Isabelle/HOL formalisation of $\FFOL$ (without description) in $\HOL$ \label{pic1}}
\end{figure}
...
abbreviation I where \[\id i \ddef (\forall x\lambdot \ex(i \comp x) \imp i \comp
...
variables $C$ and $D$, this can be encoded in
 ...
 $$\exists C, D\lambdot (\forall y\lambdot \id (C y) \wedge (C y) \comp y \cong y)
...
I. This could be done for any theory with an $\forall x\lambdot \exists i$.-axiom. The strictness axiom $S$ is extended, so
...
\subsection{Axioms Set III, IV and V} \marginpar{will improve presentation}
...
We thank G{\"u}nter Rote, Lutz Schröder and and Emil Weydert for their
